...
class _EditRoomPageState extends State<EditRoomPage> {
  ...
  void initState() {
    ...
    roomProvider.listImageUrl = widget.room.images;
    roomProvider.images = [];
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: Consumer2<UserProvider, RoomProvider>(
        builder: (context, userProvider, roomProvider, child) =>
            FloatingActionButton(
          onPressed: () {
            if (_formKey.currentState?.validate() ?? false) {
              if (roomProvider.images.isNotEmpty || listImageUrl.isNotEmpty) {
                (() async {
                  setState(() {
                    _isLoading = true;
                  });
